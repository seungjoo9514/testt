...
$('.nav > li > a').on('click', function(e){
	...
	if(navlinkparent.eq(_index).hasClass("on")) { 
		...
		navlinkparent.find('.nav_dp2').animate({height : '0'},{duration:200},{complete : function(){
	
		}});
		navlinkparent.eq(_index).find('.nav_dp2').animate({height : (lilgHT + "px")},{duration:200},{complete : function(){
...
$('.nav > li > a').hover(function(e){
	...
	var _target =  $(e.target);
	var _index = navlink.index(_target);		
	if(navlink.hasClass("on")) { 
		return false;
	} else { 
		
		navlinkparent.removeClass("on");	
		navlinkparent.eq(_index).addClass("on")
		$('.nav > li.on ul.nav_dp2').slideDown(300);
		$('.nav > li:not(.on) ul.nav_dp2').slideUp(300);			
	}
});
